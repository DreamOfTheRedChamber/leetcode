document.write('<script type="text/javascript" src="ABTreeMap.js"></script>');
...
js_cols.provide('js_cols.ABTreeMultiMap');

/**
 * 
 * js_cols.ABTreeMultiMap provides the implementation of an (a,b) Tree Multi Map datastructure. The tree
 * maintains a set of key/value pairs in a order, sorted by the keys. Contrary to an ordinary map,
 * a Multi Map allows multiple values to be mapped to the same key. The range function can be used to retrieve
 ...
 * <tt>containsKey</tt>, <tt>get</tt>, <tt>insert</tt> and <tt>remove</tt>
 ...
 * @see js_cols.ABTreeMap
 * @constructor
 * @extends js_cols.ABTreeMap
 */

js_cols.ABTreeMultiMap = function(a,b, compare_func, maxValue){
js_cols.ABTreeMap.call(this, a,b, compare_func, maxValue);

	};
	
	
	js_cols.inherits(js_cols.ABTreeMultiMap, js_cols.ABTreeMap);
	
	js_cols.ABTreeMultiMap.prototype.count_ = 0;
		/**
	...
	js_cols.ABTreeMultiMap.prototype.getCount = function(){
		return this.count_;
	}
	...
	* @private
	*/
	js_cols.ABTreeMultiMap.prototype.addToLeafs = function(leaf, k, element){
		if (this.compare(leaf.key, k)==0) {
			...
			if (this.compare(k , leaf.key)<0){
				//while(this.compare(leaf.previous.key , leaf.key)==0) leaf = leaf.previous;
				this.LeafList.insertBefore(leaf, newNode);
				

			}
			else {
				//while(this.compare(leaf.next.key , leaf.key)==0) leaf = leaf.next;
	...
	js_cols.ABTreeMultiMap.prototype.getMin = function(){
	...
	js_cols.ABTreeMultiMap.prototype.getMax = function(){
	 ...
	 * @public
	 */
	js_cols.ABTreeMultiMap.prototype.get = function(key){
	...
	/**
	 * Finds and returns the value associated with the preceeding key of that passed to the function
	 * @param {*} key
	 * @return {*} the first value associated with the preceeding key or null if the supplied key is not in the map
	 * @public
	 */
	js_cols.ABTreeMultiMap.prototype.predecessor = function(key){
	 ...
	 * @param {*} key
	 * @return {*} the first value associated with the succeeding key or null if the supplied key is not in the map
	 * @public
	 */
	js_cols.ABTreeMultiMap.prototype.successor = function(key){
		if (!this.contains(key)) return null;
	 ...
	 * @private
	 */	
	js_cols.ABTreeMultiMap.prototype.removeElement = function(leaf, parent, i){
		this.count_--;
		if (!leaf.sub) {
	...
	js_cols.ABTreeMultiMap.prototype.traverse =
		function(func) {
	...
	js_cols.ABTreeMultiMap.prototype.traverseTo =
		function(func, toKey) {
		var node = this.LeafList.sentinel.next;
	...
	js_cols.ABTreeMultiMap.prototype.traverseFromTo =
	 ...
	 * @public
	 */
	js_cols.ABTreeMultiMap.prototype.traverseBackwards =
	...
	/**
	 * Calls a function on each item in the ABTreeMultiMap.
	 *
	 * @param {Function} f The function to call for each item. The function takes
	 *     three arguments: the value, the key, and the ABTreeMultiMap.
	 * @param {Object=} opt_obj The object context to use as "this" for the
	 *     function.
	 */
	js_cols.ABTreeMultiMap.prototype.forEach = function(f, opt_obj) {
	...
	/**
	 * Calls a function on each item in the ABTreeMultiMap and returns true if any of
	 * those function calls returns a true-like value.
	 *
	 * @param {Function} f The function to call for each item. The function takes
	 *     three arguments: the value, the key and the ABTreeMultiMap, and returns a
	 *     boolean.
	 * @param {Object=} opt_obj The object context to use as "this" for the
	 *     function.
	 ...
	 *     ABTreeMultiMap.
	 */
	js_cols.ABTreeMultiMap.prototype.some = function(f, opt_obj) {
		for (var n = this.LeafList.sentinel.next; n != this.LeafList.sentinel; n = n.next) {
			var s = n;
			while (s){
	...
	/**
	 * Calls a function on each item in the ABTreeMultiMap and returns true only if every
	 ...
	 * @param {Object=} opt_obj The object context to use as "this" for the
	 *     function.
	 * @return {boolean} Whether f evaluates to true for every item in the ABTreeMultiMap.
	 */
	js_cols.ABTreeMultiMap.prototype.every = function(f, opt_obj) {
		for (var n = this.LeafList.sentinel.next; n != this.LeafList.sentinel; n = n.next) {
			var s = n;
			while (s){
			if (!f.call(opt_obj,s.data, n.key, this)) {
	...
	/**
	 * Calls a function on each item in the ABTreeMultiMap and returns the results of
	 ...
	 *     ABTreeMultiMap.
	 */
	js_cols.ABTreeMultiMap.prototype.map = function(f, opt_obj) {
		var rv = [];
		for (var n = this.LeafList.sentinel.next; n != this.LeafList.sentinel; n = n.next) {
		var s = n;
			while (s){
	...
	/**
	 * Calls a function on each item in the ABTreeMultiMap, if the function returns true, the key/value pair
	 ...
	 * @param {Object=} opt_obj The object context to use as "this" for the
	 *     function.
	 * @return {js_cols.ABTreeMultiMap} a new map with the key / value pairs that evaluated to true in the function calls 
	 *     
	 */
	js_cols.ABTreeMultiMap.prototype.filter = function(f, opt_obj) {
		var rv = new js_cols.ABTreeMultiMap(this.a, this.b, this.compare, this.LeafList.sentinel.key );
		for (var n = this.LeafList.sentinel.next; n != this.LeafList.sentinel; n = n.next) {
		var s = n;
			while (s){
			if (f.call(opt_obj, s.data, n.key, this)) {
				rv.insert(n.key, s.data);
			}
			s = s.sub;
		}
		}
	 ...
	 */
	js_cols.ABTreeMultiMap.prototype.isSubmapOf = undefined; 
	
	/**
	 * this opreation is not supported by Multi Maps
	 */
	js_cols.ABTreeMultiMap.prototype.intersection = undefined;
	
		/**
	 * Clones a map and returns a new map.
	 * @return {!js_cols.ABTreeMultiMap} A new map with the same key-value pairs.
	 */
	js_cols.ABTreeMultiMap.prototype.clone = function(){
		var rv = new js_cols.ABTreeMultiMap(this.a, this.b, this.compare, this.LeafList.sentinel.key );
		rv.insertAll(this);
		return rv;
	 ...
	 * @private
	 */
	js_cols.ABTreeMultiMap.prototype.iterator = undefined;
